...
class PostAnalysisAgent:
    ...
    def detect_actions(self, content: str, question: str, response: str) -> List[Dict]:
        """
        Détecte les actions à effectuer à partir du contenu analysé.
        
        Args:
            content: Contenu brut du document
            question: Question posée
            response: Réponse d'Ollama (analyse)
        ...
        # ============================================
        # ✅ DÉTECTION 1 : EMAILS (version améliorée)
        # ============================================
        ...
        # Exclure les emails système
        excluded_emails = [self.email_reply_to, self.email_from, '']
        #emails = [e for e in emails if e and e not in excluded_emails]
        ...
        if emails:
            actions.append({
                ...
                'data': {
                    'recipients': emails,
                    'subject': f"Réponse automatique - Document traité",
                    'context': response  # ← Réponse complète d'Ollama
                }
            })
        ...
        if dates:
            ...
            print(f"📅 {len(set(dates))} date(s) détectée(s): {list(set(dates))[:3]}")
        ...
        if amounts:
            ...
            print(f"💰 {len(set(amounts))} montant(s) détecté(s): {list(set(amounts))[:3]}")
    ...
    def _send_email(self, data: Dict, filename: str) -> Dict:
        """Envoie un email via Resend à tous les destinataires détectés"""
        ...
        # Filtrer l'email de l'expéditeur pour éviter les boucles
        recipients = [r for r in recipients if r != self.email_reply_to]
        
        if not recipients:
            return {'action': 'send_email', 'success': False, 'error': 'Tous les emails sont exclus'}
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; background: #f5f5f5; padding: 20px;">
            <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
                ...
                <div style="padding: 25px;">
                    ...
                    <strong>L'équipe de traitement automatique</strong></p>
                </div>
...
"""
        ...
        # ============================================
        # ENVOI RÉEL VIA RESEND (à tous les destinataires)
        # ============================================
        try:
            sent_ids = []
            failed = []
            ...
            for recipient in recipients:
                try:
                    ...
                    print(f"❌ Échec envoi à {recipient}: {e}")
    ...
    def _record_amount(self, data: Dict, filename: str) -> Dict:
        """Enregistre les montants détectés dans un fichier CSV"""
    ...
    def _notify_manager(self, data: Dict, filename: str) -> Dict:
        """Notifie le responsable (log + fichier)"""
        try:
    ...
    def _classify(self, data: Dict, filename: str) -> Dict:
        """Classe le document dans un dossier par type"""
